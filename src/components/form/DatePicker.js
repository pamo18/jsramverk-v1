...
class DatePicker extends Component {
    ...
    getMonths() {
        let options = [];
        let allMonths = utils.getMonthNames();
    ...
    initMonth() {
        ...
        let currentMonth = utils.getMonthSetup(selectedYear, selectedMonth);
        let startDay = utils.getDayPos(currentMonth.start);
        let endDay = currentMonth.end;
        let day;
        let week;
        let weeks = {
            1: [],
        ...
        for (let i = 0; i < startDay; i++) {
            weeks[1].push(<td class="dates empty"></td>);
        }
        for (day = 1; day <= (7 - startDay); day++) {
            ...
            if (utils.isToday(currentDate)) {
                weeks[1].push(<td class="dates today" onClick={() => this.selectDate(dateSting)}>{currentDate.getDate()}</td>);
            } else {
                weeks[1].push(<td class="dates" onClick={() => this.selectDate(dateSting)}>{currentDate.getDate()}</td>);
            }
        ...
        for (week = 2; week <= 6; week++) {
            for (let i = 1; i <= 7; i++) {
                if (day <= endDay) {
                    ...
                    let dateSting = utils.getDateAsString(currentDate);
                    if (utils.isToday(currentDate)) {
                        weeks[week].push(<td class="dates today" onClick={() => this.selectDate(dateSting)}>{currentDate.getDate()}</td>);
                    } else {
                        weeks[week].push(<td class="dates" onClick={() => this.selectDate(dateSting)}>{currentDate.getDate()}</td>);
                    }

                } else {
                    weeks[week].push(<td class="dates empty"></td>);
                }
                day++;
            }
        }
